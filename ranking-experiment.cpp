 ...
 * @authors Hussein Hazimeh and Marcus A. Streips
...
// pl2_ranker is a class that implements the pl2 ranking function. It is derived from the base class ranker.
...
public:
    ...
    pl2_ranker(double c, double lambda); // Constructor which can set parameter values
    void set_param(double c, double lambda){c_ = c; lambda_ = lambda;}; // Setter
    double score_one(const index::score_data&); // Calculates the score for a single matched term
};

const std::string pl2_ranker::id = "pl2"; // Used to identify pl2_ranker in config.toml
pl2_ranker::pl2_ranker(){}
pl2_ranker::pl2_ranker(double c, double lambda) : c_{c}, lambda_{lambda} {}

double pl2_ranker::score_one(const index::score_data& sd) 
{
    /*
    ...
    */
    ...
    double pi = 3.14; // Use this for pi - Do NOT use other values
    double lambda = lambda_; // pl2's parameter
    double c = c_; // pl2's parameter

    /* You should implement the ranking function after this comment.
       Use all the variables provided above (doc_len, avg_dl, tf, pi, lambda, and c)
       Use the function log2() to implement the logarithm
       Use exp(1) to implement e
    */

    double tfn = tf*log2(1.0+c*avg_dl/doc_len);
    
    double score = 1.0/(tfn+1.0)*(tfn*log2(tfn/lambda)+(lambda+1.0/(12.0*tfn)-tfn)*log2(exp(1)) + 0.5*log2(2.0*pi*tfn));
...
void pl2_tune (const std::shared_ptr<index::dblru_inverted_index> & idx, std::vector<corpus::document> & allqueries, index::ir_eval & eval, double & c, double & lambda)
{
    /*
    This function prints the best MAP and the corresponding c and lambda by testing different combinations of the parameters.
    idx is the inverted index
    allqueries is a vector containing all the queries found in moocs-queries.txt
    eval is an instance of ir_eval used to calculate the MAP (makes use of qrel.txt)
    */

    double cvalues [3] = {0.3, 0.6, 0.9}; // Different values for the parameter c
    double lambdavalues [8] = {0.000001,0.00001,0.0001,0.001,0.01,0.1,1,10}; // Different values for the parameter lambda
    double maxmap = 0; // Stores the current maximum MAP value
    double cmax = 7; // Stores the current optimal c (i.e. c that achieves max MAP) - Ignore the initial value
    double lambdamax = 0.1; // Stores the current optimal lambda - Ignore the initial value
    auto ranker = make_unique<pl2_ranker>(); // creates a pointer to a pl2_ranker instance
        ...
        for (int j=0 ; j<8 ; j++) // Loops over all lambda values
         {
            ...
            ranker->set_param(cvalues[i],lambdavalues[j]); // Sets the parameters of ranker to the current values of c and lambda
            ...
            {
                auto ranking = ranker->score(*idx, *query, 1000); // Returns a ranked list of the top 1000 documents for the current query
                eval.avg_p(ranking,(*query).id(),1000); // eval.avg_p stores the value of average precision for the current query in the instance eval
            }
    ...
    std::cout<<"Max MAP = "<< maxmap << " achieved by " << "c = " << cmax << ", lambda = " << lambdamax << std::endl; // Prints to the standard ouput
...
std::unique_ptr<ranker>make_ranker<pl2_ranker>(
        const cpptoml::table & config) // Used by pl2_ranker to read the parameters c and lambda from config.toml - You can ignore it
{
    double c = 7;
...
{
    index::register_ranker<pl2_ranker>(); // Registers pl2_ranker so that you can use it in config.toml

    if (argc != 2 && argc != 3)
    {
        std::cerr << "Usage:\t" << argv[0] << " configFile" << " (Task#)" << std::endl;
        return 1;
    }

    if (argc == 3 && !(strcmp(argv[2],"task4")==0 || strcmp(argv[2],"task5")==0 || strcmp(argv[2],"task6")==0 || strcmp(argv[2],"task7")==0)){
    ...
    if (argc == 3)
    {
        ...
        double token = distribution(generator);
        submission << std::setprecision(10) << token <<"\n";}
        catch(...){submission << "10.00000" <<"\n";}
    }
    ...
    std::string content;
    if (argc==3 && strcmp(argv[2],"task7")==0) // For task 7, pl2_tune is called.
    {
    ...
    // For the rest of the tasks, the top 10 documents and Precision@10 is printed for each test query, and finally the MAP is printed.
    auto elapsed_seconds = common::time([&]()
    {
        ...
        while (queries.good())
        {
            ...
            corpus::document query{"", doc_id{i-1}};
            query.content(content);
            std::cout << "Ranking query " << i++ << ": " << content
                      << std::endl;
            auto ranking = ranker->score(*idx, query, 1000);
            std::cout<< "Precision@10 for this query: "<< eval.precision(ranking,query.id(),10) << std::endl;
            if (argc == 3) {submission<<std::setprecision(5)<<ranking[0].second<<'\n';}
            eval.avg_p(ranking,query.id(),1000);
    ...
    if (argc == 3) {submission<<std::setprecision(5)<<eval.map(); submission.close();}
